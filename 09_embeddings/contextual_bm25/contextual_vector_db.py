...
class ContextualVectorDB:
    ...
    #             texts[i : i + batch_size], model="voyage-2"
    #         ).embeddings
    ...
    def _embed_and_store(self, texts: List[str], data: List[Dict[str, Any]]):
        ...
        result = [
            OpenAIEmbeddings.create(
                input=texts[i : i + batch_size], model="text-embedding-ada-002"
            )["data"]
            for i in range(0, len(texts), batch_size)
        ]
        # Extract embeddings from the OpenAI API response
        self.embeddings = [
            embedding["embedding"] for batch in result for embedding in batch
        ]
        self.metadata = data
